...
sub login :Path('/login') Args(0){
    ...
    if($c->user_exists){
        $c->response->redirect($c->uri_for(
            $c->controller('Users')->action_for('show',[$c->user->id])));
    }
    ...
    if ($username && $password) {
        ...
        if ($c->authenticate({ username => $username,
            ...
            $c->response->redirect($c->uri_for(
                $c->controller('Posts')->action_for('index')));
            return;
        } else {
...
sub logout :Path('/logout') :Args(0) {
    ...
    $c->logout;
    $c->flash(status_msg => "See you agian soon");
...
sub reguser :Path('/reguser') Args(0){
    my ( $self, $c ) = @_;
    if($c->user_exists){
        $c->response->redirect($c->uri_for(
            $c->controller('Users')->action_for('show',[$c->user->id])));
    ...
    $c->flash->{status_msg} = "Your Acount created please login";
    $c->response->redirect($c->uri_for('/login'));
